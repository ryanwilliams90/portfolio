...
- **Wire format: in-toto Statement / DSSE envelope.** The in-toto Statement model binds an attestation to one or more subjects by digest, which is exactly the shape needed here. DSSE is the recommended envelope format — it signs the serialized Statement payload together with an authenticated payload type, avoiding a security dependency on JSON canonicalization. cosign produces and verifies in-toto attestations and supports CUE / Rego policy validation, so the tooling for the predicate-and-subject pattern already exists.
